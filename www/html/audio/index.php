...
<head>
  ...
  <title>OGKB - Audio Library</title>
  ...
  <style>
    ...
    audio{ width:100%; max-width:760px; display:block; margin-top:6px; }
  </style>
...
<body>
  <header>
    <div class="topbar">
      <div>
        <h1>OGKB Audio Library</h1>
        <p class="muted">Folder-first navigation. Chapters are folders inside <code>/audio</code>.</p>
      </div>
      <div>
        <a class="pill" href="../">Home</a>
        <a class="pill" href="./">Audio Library</a>
  ...
  <main>
    ...
    <?php if ($chapterName === null): ?>
      <div class="card"><div class="muted">Select a chapter:</div></div>

      <?php if (count($chapters) === 0): ?>
        <p class="empty">No chapter folders yet. Create folders inside <code>/var/www/html/audio</code>.</p>
      <?php else: ?>
        <div class="grid">
          <?php foreach ($chapters as $ch): ?>
            ...
            <a class="chap" href="./?chapter=<?= h(rawurlencode($ch)) ?>">
              ...
              <div class="meta"><?= (int)$count ?> file<?= ($count === 1 ? '' : 's') ?></div>
            </a>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
    ...
    <?php else: ?>
      <div class="card">
        <div class="topbar">
          ...
          <div><a class="pill" href="./">Back to chapters</a></div>
        </div>
      ...
      <?php if (count($files) === 0): ?>
        <p class="empty">No audio files in this chapter yet.</p>
      <?php else: ?>
        <div class="card">
          <ul id="fileList">
            <?php foreach ($files as $f): ?>
              <li data-fname="<?= h($f) ?>">
                <a class="fileLink" href="<?= h(rawurlencode($chapterName) . '/' . rawurlencode($f)) ?>"><?= h($f) ?></a>
                <audio controls preload="none" src="<?= h(rawurlencode($chapterName) . '/' . rawurlencode($f)) ?>"></audio>
              </li>
            <?php endforeach; ?>
          </ul>
        </div>
